# alerting.tf — observability & alerting
...
# AWS will send a "Confirm subscription" email that must be clicked before it works.
...
resource "aws_cloudwatch_metric_alarm" "high_token_usage" {
  ...
  comparison_operator = "GreaterThanThreshold"
  treat_missing_data  = "notBreaching"
  alarm_actions       = [aws_sns_topic.alerts.arn] # reuse the email topic
}
